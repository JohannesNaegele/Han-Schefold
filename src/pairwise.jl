...
function pairwise_comparisons(ids; step = 0.01, save_q = true, n_goods = 36)
    ...
    Threads.@threads for i in eachindex(ids)
        ...
        model_x_trunc = create_intensities_solver(solver, l_trunc, C_trunc, ones(n_goods), lb[1:n_goods])
        ...
        for country2 in view(ids, i+1:length(ids))
            ...
            insertcols!(df_q, 1, "Country" => reduce(vcat, fill(country, n_goods) for country in [country1, country2]))
            if save_q
